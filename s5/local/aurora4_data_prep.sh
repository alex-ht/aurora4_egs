...
CORPUS=$2
WSJ0DIR=
if [ -d $CORPUS/wsj0 ]; then
  WSJ0DIR=$CORPUS/wsj0
else
  WSJ0DIR=$CORPUS/11-13.1/wsj0
fi
...
cp $WSJ0DIR/doc/lng_modl/vocab/wfl_64.lst $lmdir || exit 1;
...
cp $WSJ0DIR/doc/lng_modl/base_lm/bcb20onp.z $lmdir/lm_bg.arpa.gz || exit 1;
chmod u+w $lmdir/lm_bg.arpa.gz

# trigram would be:
cat $WSJ0DIR/doc/lng_modl/base_lm/tcb20onp.z | \
...
cp $WSJ0DIR/doc/lng_modl/base_lm/bcb05onp.z  $lmdir/lm_bg_5k.arpa.gz || exit 1;
chmod u+w $lmdir/lm_bg_5k.arpa.gz

# trigram would be: !only closed vocabulary here!
cp $WSJ0DIR/doc/lng_modl/base_lm/tcb05cnp.z $lmdir/lm_tg_5k.arpa.gz || exit 1;
